 ...
 * Handles \'XX hex, \uNNNN? Unicode, \par/\line, and plain ASCII.
 * Used on Linux/Render where textutil is not available.
 * RTF wraps the whole document in one group {\rtf1 ... }; we must not skip that
 * top-level group, only nested groups (e.g. \fonttbl{...}).
 */
  ...
  while (i < len) {
    ...
    if (ch === '\\') {
      ...
      if (next === '{' || next === '}' || next === '\\') {
        ...
        if (i < len && binary[i] === ' ') i++;
        // Unicode: \uNNNN? (optional fallback char after ?)
        if (word === 'u' && num !== '') {
          const code = parseInt(num, 10);
          if (!isNaN(code)) {
            const unicode = code < 0 ? code + 65536 : code;
            if (unicode <= 0xFFFF) text += String.fromCharCode(unicode);
            else text += String.fromCodePoint(unicode);
          }
          if (i < len && binary[i] === '?') i++;
    ...
    if (ch === '{') {
      // Only skip nested groups (e.g. \fonttbl{...}). The first { is the document wrapper.

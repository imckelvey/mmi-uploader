...
<p style="margin: 0; padding: 0 0 100px 0; color: ${c.fg} !important; font-family: 'Rubik', 'Source Sans Pro', 'Helvetica Neue', 'Arial', 'sans-serif' !important;">${escapeHtml(item.label || '')}</p>
  ...
  (parsed.usRecapRows || []).forEach(r => {
    const key = (r.name || '').toLowerCase().replace(/\s*\/\s*tsx.*/i, '').trim().replace(/\s+/g, ' ');
  ...
  const replacements = usOrder.map((key, idx) => {
    ...
    return {
      month: String((r && r.month) != null ? r.month : defaults[idx].month).trim(),
      ytd: String((r && r.ytd) != null ? r.ytd : defaults[idx].ytd).trim(),
    };
  });
  const columnNames = ['S&#38;P 500', 'Nasdaq', 'Russell 2000', '10-Year Treasury'];
  const esc = (s) => String(s).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  columnNames.forEach((colName, idx) => {
    const r = replacements[idx];
    const monthArrow = r.month.startsWith('-') ? arrowDown : arrowUp;
    // Month cell: replace any number after market_name (template values can be anything)
    out = out.replace(
      new RegExp(`(<td class="market_name"[^>]*>${esc(colName)}<\\/td>\\s*<\\/tr>\\s*<tr>\\s*<td[^>]*>)(<img[^>]+\\/>)(-?[\\d.]+)(<\\/td>)`),
      `$1${monthArrow}${r.month}$4`
    );
  });
  // YTD cells: each column has structure ... <td>&#8204;</td></tr><tr><td...><img.../>NUMBER</td> — replace NUMBER by column
  columnNames.forEach((colName, idx) => {
    const r = replacements[idx];
    const ytdArrow = r.ytd.startsWith('-') ? arrowDown : arrowUp;
    out = out.replace(
      new RegExp(`(<td class="market_name"[^>]*>${esc(colName)}<\\/td>[\\s\\S]*?<td>&#8204;<\\/td>\\s*<\\/tr>\\s*<tr>\\s*<td[^>]*>)(<img[^>]+\\/>)(-?[\\d.]+)(<\\/td>)`),
      `$1${ytdArrow}${r.ytd}$4`
    );
  });

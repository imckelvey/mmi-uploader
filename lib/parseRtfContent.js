  ...
  while (i < lines.length) {
    ...
    if (line === 'U.S. and Canadian Markets' || line === 'U.S. Markets' || line === 'U.S. and Canadian markets') {
      ...
      const isRecapStart = (idx) => {
        const l = lines[idx];
        if (l === 'S&P 500' && idx + 2 < lines.length && parseNum(lines[idx + 1]) !== null && parseNum(lines[idx + 2]) !== null) return true;
        if (l && l.match(/^U\.S\.\s*Market\s*Recap\s+for\s+/i) && idx + 1 < lines.length) return true;
        return false;
      };
      const isRecapHeader = (l) => l === 'Markets Recap' || (l && l.match(/^U\.S\.\s*Market\s*Recap\s+for\s+/i));
      while (i < lines.length && !isRecapHeader(lines[i]) && !isRecapStart(i)) {
        const l = lines[i];
        if (!l) { i++; continue; }
      ...
      // Parse US recap if we broke on "Markets Recap", "U.S. Market Recap for ...", or "S&P 500" block
      if (i < lines.length && (isRecapHeader(lines[i]) || isRecapStart(i))) {
        if (isRecapHeader(lines[i])) i++;
        while (i < lines.length && !lines[i].match(/^Yahoo Finance/)) {
          ...
          if (parts.length >= 3) {
            const name = parts[0];
            const n2 = parseNum(parts[1]);
            const n3 = parseNum(parts[2]);
            if (name.length >= 2 && name.length < 55 && (n2 !== null || n3 !== null)) {
              result.usRecapRows.push({ name, month: String(parts[1] || '').trim(), ytd: String(parts[2] || '').trim() });
              i++;
              continue;
            }
          }
    ...
    if ((line === 'Markets Recap' || line.match(/^U\.S\.\s*Market\s*Recap\s+for\s+/i)) && i < lines.length) {
      i++;
      while (i < lines.length && !lines[i].match(/^Yahoo Finance/)) {
        const l = lines[i];
        if (!l) { i++; continue; }
    ...
    if (line === 'World Markets') {
      ...
      const isWorldRecapStart = (l) => l === 'Markets Recap' || l === 'Index' || (l && l.match(/^World\s*Market\s*Recap\s+for\s+/i));
      if (i < lines.length && isWorldRecapStart(lines[i])) {
        i++;
        ...
        while (i < lines.length && !lines[i].match(/^Yahoo Finance/) && !worldSectionStops.test(lines[i] || '')) {
          ...
          if (parts.length >= 3) {
            ...
            const v1 = parts[1];
            const v2 = parts[2];
            if (isIndexName(name) && isNumericLine(v1) && isNumericLine(v2)) {
              result.worldRecapRows.push({ name, month: v1, ytd: v2 });
              i++;
              continue;
            }
            if (isNumericLine(parts[0]) && isNumericLine(parts[1]) && isIndexName(parts[2])) {
              result.worldRecapRows.push({ name: parts[2], month: parts[0], ytd: parts[1] });
              i++;
              continue;
            }
          }

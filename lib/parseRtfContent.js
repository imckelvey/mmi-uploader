...
function parseRtfContent(text) {
  ...
  while (i < lines.length) {
    ...
    if (line === 'World Markets') {
      i++;
      const narrative = [];
      while (i < lines.length && lines[i] !== 'Markets Recap' && !lines[i].match(/^Index$/) && !lines[i].match(/^World Market Recap\s*$/i)) {
        if (lines[i]) narrative.push(lines[i]);
        i++;
      }
      result.worldNarrative = narrative.join('\n\n');
      if (i < lines.length && (lines[i] === 'Markets Recap' || lines[i].match(/^Index$/) || lines[i].match(/^World Market Recap\s*$/i))) {
        i++;
